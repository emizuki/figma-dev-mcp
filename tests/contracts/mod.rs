...
fn error_code_all_is_the_set_the_enum_declares() {
    ...
    // from this same protocol, and
    // `production_plugin_source_spells_no_canonical_error_message` in
    // tests/policy/read_only.rs holds the plugin to importing from it. That is
    // a test rather than a claim here because the claim was made twice and was
    // wrong both times.
...
/// The hand-written decoder must *accept* `unresolved`, not merely tolerate it.
///
/// Every other layer of this field fails loudly on its own: the schema by the
/// two snapshots and the fingerprint, the plugin's producer and its outbound
/// validator by their own suites. The `Deserialize` generated by
/// `impl_detail_result_deserialize!` is the one hand-rolled layer and had
/// nothing. Renaming its field identifier, so the wire key is no longer
/// recognised, is a one-line change that leaves the rest of this suite green
/// while every real `unresolved` frame the plugin sends fails to decode at the
/// broker and errors the whole call — strictly worse than the silent drop this
/// field was added to replace. The rare case the field exists to serve is
/// exactly the case that would break, so it is pinned here.
///
/// Round-tripping rather than only decoding: a decoder that accepted the key
/// and discarded the value would pass a bare `is_ok`.
#[test]
fn the_forest_results_decode_an_unresolved_entry_and_keep_it() {
    let unresolved = json!([{
        "id": "1:9",
        "error": {
            "code": "LIMIT_EXCEEDED",
    ...
    let selection: GetSelectionResult = serde_json::from_value(json!({
        "detail": "minimal", "nodes": [minimal_node_fixture()],
        "unresolved": unresolved.clone(),
        "truncated": false, "observation": observation_fixture()
    }))
    .expect("a selection result carrying an unresolved entry must decode");
    let selection = serde_json::to_value(selection).unwrap();
    assert_eq!(
        selection["unresolved"], unresolved,
        "the entry must survive the round trip, not merely fail to error"
    );
    assert_eq!(selection["nodes"][0]["summary"]["id"], "1:2");

    let context: GetDesignContextResult = serde_json::from_value(json!({
        "detail": "minimal", "roots": [minimal_node_fixture()],
        "unresolved": unresolved.clone(),
        "truncated": false, "observation": observation_fixture()
    }))
    .expect("a design-context result carrying an unresolved entry must decode");
    let context = serde_json::to_value(context).unwrap();
    assert_eq!(context["unresolved"], unresolved);
    assert_eq!(context["roots"][0]["summary"]["id"], "1:2");

    // The other half of the contract: nothing to report means no key at all, so
    // a healthy read stays byte-identical to what it was before the field
    // existed.
    let healthy: GetSelectionResult = serde_json::from_value(json!({
        "detail": "minimal", "nodes": [], "unresolved": [],
        "truncated": false, "observation": observation_fixture()
    }))
    .unwrap();
    let healthy = serde_json::to_value(healthy).unwrap();
    assert!(
        healthy.get("unresolved").is_none(),
        "an empty list must be omitted entirely, not sent as []"
    );

    // `get_nodes` reports a failure per item inside its own batch, so it has no
    // such field and must refuse the key rather than ignore it.
    assert!(
        serde_json::from_value::<GetNodesResult>(json!({
            "detail": "minimal", "items": [], "unresolved": unresolved,
            "truncated": false, "observation": observation_fixture()
        }))
        .is_err(),
        "get_nodes must not accept a field it can never populate"
    );
}
...
const EXPECTED_WIRE_FINGERPRINT: &str = "0x2232f75abffae3a3";
