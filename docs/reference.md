...
Effects and paints that are switched off in Figma are not returned at all, so `effects` and `paints` list only what renders. An empty array means nothing is painted there — never "there is something we could not describe". A paint or effect type this server cannot model is reported as `{ "type": "unsupported", "figmaType": "..." }`, carrying Figma's own name for it. A node whose every stroke is switched off reports no `strokes` field. The same rule governs `get_styles`: a paint or effect style whose contents are all switched off returns `paints: []` or `effects: []`, not a style-level error.

Nodes follow that same rule: a node that does not render is not returned at all. That is judged on the whole ancestor chain rather than the node's own `visible`, so a switched-on node inside a switched-off frame is absent too, the same way a visible node inside a hidden frame does not render. There is no flag to ask for hidden content.

Naming a switched-off node directly is refused, but the shape of the refusal depends on the tool's result. `get_nodes` and `get_screenshot` return a batch, so each carries a per-item `NODE_NOT_VISIBLE` — "The requested node exists but is switched off, so it renders nothing." — and the rest of the batch still succeeds. `search_nodes` takes a single scope rather than a batch, so it rejects the whole call instead. `get_selection`'s `nodes` and `get_design_context`'s `roots` are node forests with no per-node error slot, so a switched-off root there, or a switched-off node reached deeper in scope, is excluded from the result silently, with no error at all — an empty result from either tool can mean "nothing there" or "everything present is switched off," and the response alone does not say which. The same silent exclusion governs every other tool that takes a `selector`: `get_styles`, `get_variables`, `get_components`, `get_fonts`, `get_reactions`, `get_motion`, and `get_dev_mode_data` all resolve their scope through the same root-finding step `get_design_context` uses, so a `selector` naming a switched-off node yields that tool's own ordinary empty result — no styles, no variables, no reactions — rather than an error, indistinguishable from a scope that genuinely has none of that content. `NODE_NOT_VISIBLE` is deliberately not `NODE_NOT_FOUND`: the id is correct, and re-checking it would waste the caller's time.

A second answer is possible and means something different. If the server cannot walk a node's ancestor chain at all — the chain loops, or runs deeper than the walk's limit — it cannot establish whether that node renders, and it says so rather than guessing. `get_nodes` and `get_screenshot` report `LIMIT_EXCEEDED` per item, `search_nodes` rejects the call, and `get_selection` and `get_design_context` list the node under `unresolved`, a field that is absent whenever there is nothing to report. Only this case appears there: a switched-off node is simply absent, because it is not an error and a list of every hidden layer would be noise in every response. The seven `selector`-taking tools are the exception, and it runs the other way: `get_styles`, `get_variables`, `get_components`, `get_fonts`, `get_reactions`, `get_motion`, and `get_dev_mode_data` resolve their scope through the same root-finding step but have no `unresolved` field to put the answer in, so they discard it. An unwalkable scope reaches those seven as the same ordinary empty result a switched-off one does, and the two are indistinguishable there.

On `get_screenshot` and `search_nodes`, `LIMIT_EXCEEDED` therefore carries two meanings at once. It is already the code for a budget those tools exceeded — for a raster export, the decoded byte ceiling, either side length, the pixel count, or the encoded length; for an SVG export, the source byte ceiling; and for any call, a response larger than the text or envelope limit — and it is now also the code for an ancestor chain the server could not walk. The response does not separate them. A screenshot item error is `retryable: false` in both cases and carries no other field that differs, and `search_nodes` rejects the whole call either way. The budget meaning is overwhelmingly the more likely one, so a smaller `scale` or a narrower scope is the right first response; the visibility meaning requires a document nested deeper than the ancestor walk's limit or a parent chain that loops, which a real Figma file does not produce.

To read a hidden node's content, switch it on in Figma and read again. No flag or parameter returns it while it stays hidden.
...
A node whose bounds the host will not report at all also fails this way, indistinguishable from one that renders nothing. A switched-off node never reaches this check: `get_screenshot` refuses it with `NODE_NOT_VISIBLE` before the exporter runs, whether the node is hidden by its own `visible` or an ancestor's.

Switched-off is not the only way something fails to render: a node at `opacity: 0`, a blend mode, or a transparent ancestor all still apply, and none of them are reflected in `effects` or `paints`. The decisive cross-check is the render bounds — Figma expands a node's PNG export to fit its enabled effects, so comparing `get_screenshot`'s returned `width`/`height` against the node's `bounds` measures what actually renders. That comparison assumes the default `scale: 1`; a caller-supplied `scale` multiplies every returned dimension regardless of effects, so it must be divided back out before comparing. The expansion formula is drop-shadow-only: per side it is `radius + spread`, shifted by `offsetX`/`offsetY`. An `INNER_SHADOW` paints inside the shape and expands the bounds not at all, so applying that formula to one misreads a real inner shadow as missing. A `LAYER_BLUR` expands the bounds by roughly its `radius`, with no spread or offset term; a `BACKGROUND_BLUR` does not expand the node's own bounds.
